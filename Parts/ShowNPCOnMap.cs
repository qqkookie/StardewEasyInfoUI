...
#region Members
        ...
        internal ShowNPCOnMap(IDictionary<String, String> options)
        {
            _options = options;
            ...
            this._friendNames = fl.ToArray();

            foreach (string name in _friendNames)
            {
        ...
        internal void ToggleShowNPCLocationsOnMap(bool showLocations)
        {
            ExtendMenuIfNeeded();
            Events.Display.RenderedActiveMenu -= OnRenderedActiveMenu_DrawSocialPageOptions;
            Events.Display.RenderedActiveMenu -= OnRenderedActiveMenu_DrawNPCLocationsOnMap;
        ...
        private const int which_offset = 777;
        private  string OptionKey(string friendName)
        {
           return _friendNames.Contains(friendName) ? ("onmap." + friendName) : "";
        }

        private string OptionKey(int which)
        {
            which -= which_offset;
            return (which >= 0 && which < _friendNames.Length) ?
            ("onmap." + _friendNames[which]) : "";
        }
            ...
            if (Game1.activeClickableMenu is GameMenu gameMenu && gameMenu.currentTab == 2)
            {
                ...
                for (int i = slotPosition; i < slotPosition + 5 && i < _friendNames.Length; ++i)
                {
                    ...
                    if (checkbox.bounds.Contains(Game1.getMouseX(), Game1.getMouseY()))
                        IClickableMenu.drawHoverText(
                            Game1.spriteBatch,
                            ModEntry.Translation.Get(LanguageKeys.TrackOnMap),
                            Game1.dialogueFont);
                }
            }
        }
